...
class X25519KeyShare : public SSLKeyShare {
 public:
  ...
  bool Offer(CBB *out) override {
    ...
    fprintf(log_file, "X25519_keypair public_key: ");
    for (int i = 0; i < 32; i++) {
        fprintf(log_file, "%02x", public_key[i]);
    }
    fprintf(log_file, "\n");

    fprintf(log_file, "X25519_keypair private_key_: ");
    for (int i = 0; i < 32; i++) {
        fprintf(log_file, "%02x", private_key_[i]);
    }
    fprintf(log_file, "\n");

    fclose(log_file);    return !!CBB_add_bytes(out, public_key, sizeof(public_key));
  }
